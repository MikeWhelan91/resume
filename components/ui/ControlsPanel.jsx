  ...
  return (
    <div className="space-y-6">
      ...
      <div>
        ...
        <label className="flex items-center gap-2 text-sm"><input type="checkbox" checked={atsMode} onChange={e=>setAtsMode(e.target.checked)} />ATS mode</label>
      </div>
      <div>
      ...
      <div>
        ...
        <div className="flex items-center gap-2">
          <button onClick={()=>onPageChange(Math.max(page-1,0))} disabled={page<=0}><Icon name="left" /></button>
          <span>{page+1} / {pageCount}</span>
          <button onClick={()=>onPageChange(Math.min(page+1,pageCount-1))} disabled={page>=pageCount-1}><Icon name="right" /></button>
        </div>
      </div>
    </div>
  );
