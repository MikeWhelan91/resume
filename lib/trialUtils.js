...
export async function checkTrialLimit(req, actionType) {
  ...
  try {
    ...
    // Parse, export, and other generation-related actions use generation limit
    if (actionType === 'parse' || actionType === 'export') {
      return {
        allowed: currentGenerations < 2,
        remaining: Math.max(0, 2 - currentGenerations),
        limit: 2,
...
export async function consumeTrialUsage(req, actionType) {
  ...
  try {
    ...
    const updateData = (actionType === 'generation' || actionType === 'parse' || actionType === 'export')
    ...
    const updatedUsage = await prisma.trialUsage.upsert({
      ...
      create: {
        ipAddress,
        generationsUsed: (actionType === 'generation' || actionType === 'parse' || actionType === 'export') ? 1 : 0,
