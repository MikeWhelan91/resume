...
function mapDensity(d="normal"){
  const key = String(d).toLowerCase();
  return key === 'compact' ? 'Compact'
       : key === 'cozy'    ? 'Relaxed'
       : 'Normal';
}

export function toTemplateModel(data = {}, { ats = false, density = 'normal', isCoverLetter = false } = {}) {
  const cover = data.coverLetter || {};
  const bodyHtml = String(cover.body || '')
    .split(/\n{2,}/).map(p => `<p>${p.trim().replace(/\n/g,'<br/>')}</p>`).join('');
  const today = new Date().toLocaleDateString('en-GB', { day:'2-digit', month:'long', year:'numeric' });
  const tplDensity = mapDensity(density);
