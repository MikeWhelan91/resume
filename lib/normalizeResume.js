export function normalizeResumeData(raw = {}) {
  ...
  // Map uppercase ATS field names to standard lowercase field names
  if (raw['PROFESSIONAL EXPERIENCE'] && !raw.experience?.length) {
    out.experience = raw['PROFESSIONAL EXPERIENCE'];
  }
  if (raw['CORE COMPETENCIES'] && !raw.skills?.length) {
    out.skills = raw['CORE COMPETENCIES'];
  }
  if (raw['EDUCATION'] && !raw.education?.length) {
    out.education = raw['EDUCATION'];
  }
  if (raw['PROJECTS'] && !raw.projects?.length) {
    out.projects = raw['PROJECTS'];
  }
  if (raw['PROFESSIONAL SUMMARY'] && !raw.summary?.trim()) {
    out.summary = raw['PROFESSIONAL SUMMARY'];
  }
