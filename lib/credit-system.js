...
const PLANS = {
  ...
  standard: {
    weekly_credits: null, // No weekly limit (credit-based)
    docx: false, // Keep DOCX as Pro-only feature
    templates: 'all', // Allow all templates for standard users
    colors: 'all', // Allow all colors for standard users
    max_profiles: 5,
    monthly_generation_cap: null,
    monthly_download_cap: null,
    rate_limit_rpm: 20,
...
function getEffectivePlan(entitlement) {
  if (!entitlement) return 'standard'
  // Normalize legacy values
  if (entitlement.plan === 'day_pass') return 'standard'
  if (entitlement.plan === 'free') return 'standard'
...
async function resetWeeklyCreditsIfNeeded(userId, entitlement) {
  ...
  // Legacy weekly reset is no longer used for Standard/Pro
  if (effectivePlan !== 'standard') return entitlement
...
export async function checkCreditAvailability(userId, action = 'generation') {
  ...
  if (effectivePlan === 'free') {
    if ((action === 'generation' || action === 'download') && entitlement.freeWeeklyCreditsRemaining <= 0) {
      return {
        allowed: false,
        message: 'Weekly credits exhausted. You have 0/10 credits remaining this week.',
